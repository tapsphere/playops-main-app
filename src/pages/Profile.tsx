...
import { User, Award, TrendingUp, Shield, Home, Hexagon, Wallet, Loader2 } from 'lucide-react';
...
const Profile = () => {
  ...
  const loadProfileData = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('id, user_id, full_name, location, bio, wallet_address, total_xp, total_plyo, created_at, updated_at, avatar_url')
        .eq('user_id', user.id)
        .single();
      if (error) {
